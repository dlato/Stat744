...
I chose blue so that it would be colour-blind friendly and easy to distinguish from black when a point was hovered over.
...
I chose the specific transparancy because I still wanted to highlight the fact that bacterial genomes often have genes that overlap partially or completely because they are extremely space efficent creatures when it comes to their genomes!
So, I wanted an transparent value that would still allow for sections of the graph to be densly coloured with these overlapping genes, but still for the most part have all points visible.
...
My main reason for making this simple scatterplot interactive is that it is biologically interesting.
...
For example, the gene located roughly at position (0.5 Mbp, 10000 CPM) is gene fusA.
This gene encodes an elongation factor which facilitates the translocation of teh ribosome and is essential for DNA replication.
...
Unfortunatly, I would have to do a bit more analysis and Python coding to obtain this information and add it to my dataset.
