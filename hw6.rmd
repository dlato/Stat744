...
I decided to use direct labels to avoide having the viewer rely on looking at a legend for reference.
...
I suppose this might be a preference and asthetic issue and not a "bad graph" issue.
...
I tried to choose colours that are asthetically pleasing but also easy to distinguish between the two categorical variables.
...
My favourite website to choose complementary colours from is [coolors](https://coolors.co/).
...
Another biologically curious aspect to this graph is that overall the number of protein coding substitutions seems to exceede the number of non-protein coding substitutions.
...
Bacterial genomes are almost exclusivly protein coding!
...
With respect to the visualization of this graph, most of the same points that I mentioned before appily.
...
I keept the colouring of each genomic category the same as the previous graph so that anyone reading this report would begin to associate protein coding with dark blue and non-protein coding with orange.
