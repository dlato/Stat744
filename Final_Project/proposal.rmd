...
(chromosome, pSymA, pSymB) will be analyzed separately. So we effectively have 6 bacterial datasets:
...
The substitutions dataset gives information about the number of substitutions (effecitively mutations) and the distance from the origin of replication.
...
Therefore, multiple substitutions may have occured at a particular base in the genome.
The genomic positions in this dataset have been scaled to represent base pair distance from the origin of replication, with the furthest distance from the origin of replication being the terminus of replication.

### Gene Expression Data:

The gene expression dataset has median Counts Per Milltion (CPM) expression values for each gene in the genome.
...
The selection dataset has information on the non-synonymous (dN) substitution rate, synonymous (dS) substitution rate, and $\omega$ (dN / dS) for each gene in the bacterial genomes.
...
Non-synonymous substitutions cause a change to the amino acid sequence of a gene, which could completely alter the function of the gene and be fatial to the organism.
...
The $\omega$ ratio allows us to determine if these changes in the sequence cause benificial or deleterious traits to arise.
If $\omega$ for a gene is larger than 1, the gene is under positive selection and therefore is benificial to the organism and will likely be maintained in the genome over time.
...
If $\omega$ is equal to 1, the gene is under neutral selection, and is neither benificial nor deleterious to the organism.
...
have a single linear chromosome that also replicates from a sigle origin of replication located roughly in the middle of the replicon, and continues in both directions until the ends of the chromosome are reached.
There are certain properties that are believed to be assoiciated with distance from the origin of replication.
...
Selective pressures at the termins should be positive, because this region has higher mutation rates and is often recombining, there should be selective pressue to increase benificial traits. 
...
This leaves us with three predictions for our datasets:
...
3. Genes under neutral or purifying selection should be found near the origin of replication, any genes that are under positive selection should be located near the terminus