...
In this graphic the coefficent estimates for each logistic regression are plotted.
I have added in a reference line at zero to assist with determining statistical significance. 
Typically, any coefficent estimates (and their respective standard errors) that cross over the zero reference line are not considered significant.
...
, the coefficent estimate is very close to the zero reference line, but does not touch it, and is therefore significant.

It is also very easy to see which bacterial replicons have a positive or negative coefficent estimate. 
pSymB of \smel 
and \strep 
are the only replicons to have a significant positive coefficent estimate, while the rest of the bacterial replicons have a significant negative coefficent estimate.

### Discussion on Biological Relevance
If the coefficent estimate is significantly positive, in this case it means that the probability of a substitution at a particular base increases with increasing distance from the origin of replication.
...
If the coefficent estimate is significantly negative, in this case it means that the probability of a substitution at a particular base in the genome decreases with increasing distance from the origin of replication.
This is counterintutive to the typical organization of bacterial genomes.
However, I failed to mention the most important part of my analysis, I take into account genomic rearrangements.
Bacterial genomes are frequently re-arranging, re-organizing, and re-working their genomic information through a variety of processes such as duplications, traanslocations, and horizontal gene transfer.
...
dwplot(size = 3, subs_dat) %>%
  relabel_predictors(ecoli = "E.coli",
                          ...
                          pa = "S.meliloti pSymA",
                          pb = "S.meliloti pSymB", face = "italic") +
  xlab(bquote('Coefficent Estimates'~(x10^2))) +
  geom_vline(xintercept=0,lty=2)+ 
  scale_colour_manual(values=c("#E08F8F","#173FAA"),
                      name = "",
                      labels = c("Protein Coding", "Non-Protein Coding"))
...
I did not like the way that the coefficent estimate numbers looked on the x-axis so I wanted to try and see what they would look like as smaller numbers.
I think it looks better, but I am not sure if this is maybe harder to read because people have to do some mental math to determine how small the coefficent estimate is.
I am also unsure if scaling the coefficent estimates and standard errors this way is wrong.
...
It never occured to me to present my regression results like this.
